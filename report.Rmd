...
knitr::opts_chunk$set(eval = FALSE,message = FALSE,warning = FALSE, include  = FALSE)
library(caret)
library(mgcv)
library(tidyverse)
library(ggplot2)
...
The tuning parameter are n.trees(the number of trees) with optimal value 5000, interaction.depth(the complexity of boosted ensemble) with optimal value 23, shrinkage(the rate of boosting learn) with optimal value 0.005, n.minobsinnode(minimal node size) with optimal value 1 in boosting model.
...
Different from the linear kernel, radial kernel can construct nonlinear classification boundaries. The tuning parameters are cost(C) with optimal value 20.0855, sigma($\gamma$, local behavior) 0.04978 in support vector machine with radinal kernel. 
